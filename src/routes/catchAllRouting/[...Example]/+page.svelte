...
{#if locArr.length === 1}
    ...
    <button on:click={(event) => { event.preventDefault(); window.location.reload(); window.location.href = `/catchAllRouting/${location}/content`} } class="bg-green-200">Try nested catch all</button>
